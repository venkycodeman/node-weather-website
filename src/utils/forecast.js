...
const forecast = (latitude, longitude, callback) => {
    ...
    request({ url, json: true }, (error, { body }) => {
        if (error) {
            ...
            callback(undefined, `${body.current.weather_descriptions}. It is currently ${body.current.temperature} degrees out, but it feels like ${body.current.feelslike} degrees celsius. Humidity is ${body.current.humidity} percent. Wind speed is ${body.current.wind_speed} Kmph. And there is ${body.current.precip} percent chance of rain.`)
        }
    })
}
